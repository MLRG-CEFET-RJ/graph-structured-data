...
class PointerNetHelper(Helper):
  ...
  def logits_to_sequences(self, pred):
    logits_sequences = {}
    true_sequences = {}
...
class Attention(nn.Module):
    def __init__(self, use_cuda, hidden_size, use_tanh=False, C=10):
...
class PointerNetwork(ModelInterface):
  def __init__(self, NUMBER_NODES, batch_size, epochs):
    ...
    self.epochs = epochs
    self.helper = PointerNetHelper(NUMBER_NODES)

  def preprocess_data(self, X, Y):
    graph = self.helper.getGraph(X)
    Graph = nx.Graph(graph)

    A = nx.adjacency_matrix(Graph, nodelist=Y).todense()

    upperTriangleFlatten = np.array([
        A[row, column]
        for row in range(self.NUMBER_NODES - 1)
        for column in range(row + 1, self.NUMBER_NODES)
      ]  
    )

    return upperTriangleFlatten
      ...
      with torch.no_grad():
        for input_data, target_data in test_dataloader:
          for x, y in zip(input_data, target_data):
            ...
            pred, target = self.helper.logits_to_sequences(prediction)
