...
class RandomForestClassifier(ModelInterface):
  ...
  def fit(self):
    ...
    dump(model, os.path.join('saved_models', f'RandomForestClassifier_{self.NUMBER_NODES}_vertices.joblib'))
  def predict(self):
    try:
      model = load(os.path.join('saved_models', f'RandomForestClassifier_{self.NUMBER_NODES}_vertices.joblib')) 
      ...
      RandomForestClassifierResult = helper.getResult(
...
if __name__ == '__main__':
  ...
  if args.mode == '1':
    df_result = randomForestClassifier.predict()
