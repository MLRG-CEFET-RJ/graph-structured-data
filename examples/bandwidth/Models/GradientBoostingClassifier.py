...
class GradientBoostingClassifier(ModelInterface):
  ...
  def fit(self):
    ...
    dump(model, os.path.join('saved_models', f'GradientBoostingClassifier_{self.NUMBER_NODES}_vertices.joblib'))
  def predict(self):
    try:
      model = load(os.path.join('saved_models', f'GradientBoostingClassifier_{self.NUMBER_NODES}_vertices.joblib')) 
      ...
      GradientBoostingClassifierResult = helper.getResult(
...
if __name__ == '__main__':
  ...
  if args.mode == '1':
    df_result = gradientBoostingClassifier.predict()
