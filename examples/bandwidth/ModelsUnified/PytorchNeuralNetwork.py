...
class NeuralNetwork(nn.Module):
  def __init__(self, MAX_NUMBER_NODES):
      ...
      self.fc1 = nn.Linear(((MAX_NUMBER_NODES * MAX_NUMBER_NODES - MAX_NUMBER_NODES) // 2 ), 128)
      self.fc2 = nn.Linear(128, 128)
      self.fc3 = nn.Linear(128, MAX_NUMBER_NODES)
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        path = os.path.join('saved_models', f'PytorchNeuralNetwork_unified_{self.MAX_NUMBER_NODES}_vertices.pt')
...
class PytorchNeuralNetwork(ModelInterface):
  ...
  def fit(self):
    ...
    model = NeuralNetwork(self.MAX_NUMBER_NODES).to(self.device)
    ...
    plt.savefig(os.path.join('plotted_figures', f'PytorchNeuralNetwork_loss_unified_{self.MAX_NUMBER_NODES}_vertices.jpg'))
    plt.clf()

  def predict(self):
    try:
      helper = Helper(self.MAX_NUMBER_NODES)
      self.device = get_default_device()
      print(f"Using {self.device} device")
      ...
      with torch.no_grad():
        for x, y in test_dataloader:
          ...
          for features, target in zip(x, y):
            ...
            pred, quantity_repeated, cases_repeated = helper.get_pred(preds[0], target)
            ...
            target = helper.get_valid_target(target)
            ...
            original_bandwidths[target_length].append(original_band)

            pred_band = helper.get_bandwidth(graph, pred)
            pred_bandwidths[target_length].append(pred_band)

            true_band = helper.get_bandwidth(graph, target)
            target_bandwidths[target_length].append(true_band)
...
if __name__ == '__main__':
  ...
  pytorchNeuralNetwork = PytorchNeuralNetwork(MAX_NUMBER_NODES=MAX_NUMBER_NODES, batch_size=batch_size, epochs=epochs)
